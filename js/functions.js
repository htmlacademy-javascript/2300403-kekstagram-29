...
function lengthCheck (string, length) {
  let stringProtector = String(string);
...
function palindrome (string) {
  let clearString = string.replaceAll(' ', '');
  let lowerString = clearString.toLowerCase();
  let newString = '';
...
function numberExtractor (string) {
  let stringProtector = string.toString();
  let numberString = '';
  for (let stringBrute = 0; stringBrute <= stringProtector.length - 1; stringBrute++) {
    let symbolCheck = parseInt(stringProtector[stringBrute], 10);
